...
docker-compose exec config01 sh -c "mongo --port 27017 < /scripts/admin.js"
# docker-compose exec config01 sh -c "mongo --port 27017 -u root -p example --authenticationDatabase=admin < /scripts/init-db.js"
docker-compose exec router sh -c "mongo --port 27017 -u root -p example --authenticationDatabase=admin < /scripts/init-db.js"